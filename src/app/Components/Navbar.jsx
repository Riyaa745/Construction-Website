...
export default function Navbar() {
  ...
  return (
    <>
      {/* Black full screen overlay when mobile menu is open */}
      {mobileOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-40 transition-all duration-300"></div>
      )}

      <nav
        className={`fixed top-0 left-0 w-full z-50 px-0 py-5 transition-all duration-300 ${
          isHomePage && !isScrolled && !mobileOpen
            ? 'bg-transparent text-white'
            : 'bg-black/60 backdrop-blur-md text-white'
        }`}
      >
        <div className="container mx-auto px-5 flex justify-between items-center relative z-50">
          {/* Logo */}
          <Link href="/">
            <div className="w-30">
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div className="md:hidden w-full min-h-screen bg-black text-white pb-6 px-4 absolute top-full left-0 z-50">
            {[
              { label: 'Home', href: '/' },
              { label: 'About', href: '/aboutus' },
              { label: 'Service', href: '/service' },
